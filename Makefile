...
build: check_docker prep_version_incrementor ## Build the fluentbit docker image
	...
	@umask 0022
	@docker build --no-cache --tag artefacts.tax.service.gov.uk/accessibility-assessment-fluentbit:$$(cat .version) .
...
push_image: ## Push the fluentbit docker image to artifactory
	@docker push artefacts.tax.service.gov.uk/accessibility-assessment-fluentbit:$$(cat .version)
...
clean: ## Remove the fluentbit docker image
	@echo '********** Cleaning up ************'
	@docker rmi -f $$(docker images artefacts.tax.service.gov.uk/accessibility-assessment-fluentbit:$$(cat .version) -q)
